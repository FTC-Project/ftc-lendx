...
def register(name: str, *, aliases: Optional[Iterable[str]] = None,
             description: str = "", permission: str = "public"):
    """Decorator: @register('name', aliases=['/name'], permission='user')"""
    aliases = aliases or []
    def _decorator(cls: Type[BaseCommand]) -> Type[BaseCommand]:
        meta = CommandMeta(cls=cls, name=name, aliases=aliases, description=description, permission=permission)
