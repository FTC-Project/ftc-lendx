...
def process_deposit_ftct(
    self, wallet: str, private_key: str, amount: float, task_id: str = None
) -> dict:
    """
    ...
    try:
        ...
        approve_tx_hash = (
            approve_tx.get("tx_hash")
            if isinstance(approve_tx, dict)
            else str(approve_tx)
        )
        ...
        deposit_tx_hash = (
            deposit_tx.get("tx_hash")
            if isinstance(deposit_tx, dict)
            else str(deposit_tx)
        )
