        ...
        if request.method == 'GET':
            ...
            if wallet_q:
                try:
                    ...
                    xrp_balance = float(ftc_service.web3.from_wei(
                    ...
                    try:
                        ...
                        if w:
                            deposits_sum = sum(float(d.amount) for d in PoolDeposit.objects.filter(user=w.user))
                            withdrawals_sum = sum(float(wd.principal_out + wd.interest_out) for wd in PoolWithdrawal.objects.filter(user=w.user))
            ...
            active_count = Loan.objects.filter(state__in=["created", "funded", "disbursed"]).count()
