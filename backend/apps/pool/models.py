...
class PoolAccount(models.Model):
    ...
    user = models.OneToOneField(
        TelegramUser, on_delete=models.CASCADE, related_name="pool_account"
    )
    principal = models.BigIntegerField(
        default=0
    )  # wei-like minor unit if mirroring chain; POC: ZAR int
...
class PoolDeposit(models.Model):
    ...
    user = models.ForeignKey(
        TelegramUser, on_delete=models.CASCADE, related_name="pool_deposits"
    )
...
class PoolWithdrawal(models.Model):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    user = models.ForeignKey(
        TelegramUser, on_delete=models.CASCADE, related_name="pool_withdrawals"
    )
