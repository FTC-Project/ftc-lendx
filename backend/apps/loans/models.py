...
class Loan(models.Model):
    ...
    user = models.ForeignKey(TelegramUser, on_delete=models.CASCADE, related_name="loans")
    amount = models.IntegerField()  # cents or ZAR integer; choose one and be consistent; POC: ZAR integer
    term_days = models.PositiveIntegerField()
    apr_bps = models.PositiveIntegerField()  # 2500 = 25.00%
    state = models.CharField(max_length=16, choices=STATE, default="created", db_index=True)

    # Escrow/chain refs (XRPL EVM):
    contract_address = models.CharField(max_length=64, null=True, blank=True, db_index=True)
    escrow_address = models.CharField(max_length=64, null=True, blank=True, db_index=True)
...
class RepaymentSchedule(models.Model):
    ...
    status = models.CharField(max_length=16, default="pending", db_index=True)  # pending/partial/paid/late
...
class Repayment(models.Model):
    ...
    method = models.CharField(max_length=32, default="telegram")  # or bank_transfer, etc.
...
class LoanEvent(models.Model):
    ...
    name = models.CharField(max_length=32, db_index=True)  # created, funded, disbursed, repaid, defaulted, declined
...
class Disbursement(models.Model):
    ...
    status = models.CharField(max_length=16, default="pending", db_index=True)  # pending/sent/confirmed/failed
