...
class LoanSystemService(BaseContractService):
    ...
    def calculate_interest(
        self, principal: float, apr_bps: int, term_days: int
    ) -> Decimal:
        """
        ...
        interest_wei = self.call_read_function(
            "_calcInterest", principal_wei, apr_bps, term_days
        )
    ...
    def deposit_ftct(
        self, lender_address: str, amount: float, lender_private_key: str
    ) -> Dict[str, Any]:
        """
    ...
    def withdraw_ftct(
        self, lender_address: str, share_amount: float, lender_private_key: str
    ) -> Dict[str, Any]:
        """
        ...
        logger.info(
            f"Withdrawing {share_amount} shares (~{ftct_amount} FTCT) from {lender_address}"
        )
    ...
    def create_loan(
        ...
        logger.info(
            f"Creating loan: {amount} FTCT for {borrower_address}, {apr_bps}bps, {term_days}d"
        )

        function = self.contract.functions.createLoan(
            borrower_address, amount_wei, apr_bps, term_days
        )
    ...
    def mark_funded(
        self, loan_id: int, admin_private_key: Optional[str] = None
    ) -> Dict[str, Any]:
        """
    ...
    def get_deposit_events(
        self, from_block: int = 0, to_block: str = "latest", user: Optional[str] = None
    ):
