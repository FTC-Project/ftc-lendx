...
class FTCTokenService(BaseContractService):
    ...
    def mint(
        self, to_address: str, amount: float, admin_private_key: Optional[str] = None
    ) -> Dict[str, Any]:
        """
    ...
    def approve(
        self, owner_address: str, spender_address: str, amount: float, private_key: str
    ) -> Dict[str, Any]:
        """
        ...
        logger.info(
            f"Approved {amount} FTCT for {spender_address} (tx: {result['tx_hash']})"
        )
        return result
